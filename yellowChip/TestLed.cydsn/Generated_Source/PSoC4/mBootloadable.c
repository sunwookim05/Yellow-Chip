...
static cystatus mBootloadable_WriteFlashByte(const uint32 address, const uint8 inputValue) CYLARGE \
                               ;
#endif /*(CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/

/*******************************************************************************
* Function Name: mBootloadable_Load
****************************************************************************//**
*
* \brief
...
*
*******************************************************************************/
void mBootloadable_Load(void) 
{
    /* Schedule Bootloader to start after reset */
    mBootloadable_SET_RUN_TYPE(mBootloadable_SCHEDULE_BTLDR);
...
#if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
/*******************************************************************************
* Function Name: mBootloadable_GetActiveApplication
****************************************************************************//**
*
* \brief
...
{   
    uint8 result = mBootloadable_MD_BTLDB_ACTIVE_NONE;
    
    if (0u != mBootloadable_GET_CODE_DATA( \
              mBootloadable_MD_BTLDB_ACTIVE_OFFSET(mBootloadable_MD_BTLDB_ACTIVE_0))) 
    {
        result = mBootloadable_MD_BTLDB_ACTIVE_0;
    }
    else if (0u != mBootloadable_GET_CODE_DATA( \
                   mBootloadable_MD_BTLDB_ACTIVE_OFFSET(mBootloadable_MD_BTLDB_ACTIVE_1)))
    {
        result = mBootloadable_MD_BTLDB_ACTIVE_1;    
    }
    else
    {
...
/*******************************************************************************
* Function Name: mBootloadable_SetActiveApplication
****************************************************************************//**
*
* \brief 
                ...
                
{
    ...
    if (appId > mBootloadable_MD_BTLDB_ACTIVE_1)
    {
    ...
    else
    {
        ...
        if (1u == mBootloadable_GET_CODE_DATA(mBootloadable_MD_BTLDB_ACTIVE_OFFSET(appId)))
        {
            result = CYRET_BAD_PARAM;
        }
        else
        {
            ...
            for(idx = 0u; idx < mBootloadable_MAX_NUM_OF_BTLDB; idx++)
            {
                result |= mBootloadable_WriteFlashByte((uint32) mBootloadable_MD_BTLDB_ACTIVE_OFFSET(idx), \
                                                                                            (uint8)(idx == appId));
            }
        }
    }
...
/*******************************************************************************
* Function Name: mBootloadable_WriteFlashByte
****************************************************************************//**
*
* \brief 
                               ...
                               
{
    ...
    {
        rowData[idx] = (uint8)mBootloadable_GET_CODE_DATA(baseAddr + idx);
    }
...
void mBootloadable_SetFlashByte(uint32 address, uint8 runType) 
{
    ...
    for (idx = 0u; idx < CYDEV_FLS_ROW_SIZE; idx++)
    {
        rowData[idx] = mBootloadable_GET_CODE_DATA(baseAddr + idx);
    }
